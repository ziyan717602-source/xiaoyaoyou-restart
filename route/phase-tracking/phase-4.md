...
- [x] `src/game/skills/types.ts` - 技能接口定义（含 triggers/occurs 多触发器支持）
- [x] `src/game/skills/index.ts` - 技能注册表（按触发时机索引）
- [x] `src/game/skills/resolver.ts` - 技能解析器（遍历 → 检查 → 执行）
- [x] `src/game/skills/occurs.ts` - OCCURS 编码解析器（OCCURS → SkillTrigger 映射）
- [x] `src/shared/types/enums.ts` - SkillTrigger 枚举扩展（90+ 值）

### 4.2 仙剑一（7 角色，14 技能）

- [x] `src/game/skills/xianjian1.ts`
- [ ] 李逍遥：侠骨柔肠[!]、飞龙探云手[!]
- [ ] 赵灵儿：双剑、梦蛇[!]
- [ ] 赵灵儿·梦蛇：双剑、女娲[!]、变身[!]
- [ ] 林月如：林家剑法[!]、嫉恶如仇[!]
...
- [x] `src/game/skills/xianjian2.ts`
- [ ] 王小虎：发挥不稳定[!]、不屈不挠
- [ ] 苏媚：狡猾、拒绝
- [ ] 沈欺霜：仙霞五奇[!]、元灵归心术
- [ ] 孔璘：辣手摧花、生命献祭[!]
...
- [x] `src/game/skills/xianjian3.ts`
- [ ] 唐雪见：追打、连击、好胜
- [ ] 龙葵：变身、熔铸、剑灵
- [ ] 龙葵鬼：变身、控剑、剑魂
- [ ] 紫萱：关爱[!]、神圣[!]
...
- [x] `src/game/skills/xianjian4.ts`
- [ ] 云天河：天河剑[!]、后羿射日弓
- [ ] 韩菱纱：搜囊探宝、劫富济贫、盗墓[!]
- [ ] 柳梦璃：妖王[!]、梦傀儡[!]
- [ ] 慕容紫英：赠剑、剑匣[!]
...
- [x] `src/game/skills/xianjian5.ts`
- [ ] 龙幽：越行之术[!]、表现欲[!]
- [ ] 小蛮：无法无天、活力[!]、炼药

### 4.7 凤鸣玉誓扩展（6 角色，13 技能）

- [x] `src/game/skills/fengming.ts`
- [ ] 姜云凡 (XJ501)：狂龙迅影斩、山贼
- [ ] 唐雨柔 (XJ502)：咏圣调、逆天阵
- [ ] 姜世离 (XJ505)：魔君[!]、牺牲
- [ ] 魔翳 (XJ506)：锁魂[!]、底牌[!]
- [ ] 湮世穹兵 (XJ507)：侵略如火[!]、毁天灭地
- [ ] 欧阳慧 (XJ508)：雷灵、雷屏[!]、雳天击

### 4.8 变身系统

- [x] `src/game/engine/transform.ts`
- [x] 赵灵儿 ↔ 赵灵儿·梦蛇（条件触发）
- [x] 龙葵 ↔ 龙葵鬼（主动选择）
- [x] 孔璘 → 魔尊（死亡时）
- [x] 魔翳 → 湮世穹兵（队友全灭时）
...
| OCCURS 解析器 | `src/game/skills/occurs.ts` | ✅ |
| SkillTrigger 枚举 | `src/shared/types/enums.ts` | ✅ |
