...
/**
 * 检查参战者是否命中
 * 命中规则：攻击方命中(DEX + 装备命中) >= 怪物闪避(AGL)
 */
function isHit(G: GameState, playerId: string): boolean {
  if (!G.combat) return false;
  if (!isParticipant(G, playerId)) return false;
  ...
  // 命中值 = 基础DEX + 装备命中加成
  const heroHit = hero.currentDex;

  // 怪物闪避值
  const monsterAgl = monster.currentAgl;

  return heroHit >= monsterAgl;
}
