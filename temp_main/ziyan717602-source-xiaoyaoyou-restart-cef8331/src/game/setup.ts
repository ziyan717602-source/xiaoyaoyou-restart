...
import { heroes, monsters, tux, npcs, events } from '../shared/data';
import type { GameState, PlayerState } from '../shared/types/game';
import type { HeroInstance } from '../shared/types/hero';
import type { CardInstance, MonsterInstance, NpcInstance, EventInstance } from '../shared/types/card';
...
export function setup({ ctx }: { ctx: any }): GameState {
  ...
  const npcInstancesLocal: Record<string, NpcInstance> = {};
  for (let idx = 0; idx < shuffledMonsterPile.length; idx++) {
    ...
    if (monsterData) {
      ...
      // NPC 实例
      const npcData = npcs.find(n => n.ID === mId);
      if (npcData) {
        const instanceId = `npc_${mId}_${idx}`;
        const npcInst: NpcInstance = {
          instanceId,
          staticId: mId,
          currentStr: npcData.STR,
          buffs: [],
        };
        npcInstancesLocal[instanceId] = npcInst;
        monsterPile.push(instanceId);
      }
    }
  }
  ...
  // ========== 构建事件牌堆 ==========
  const validEvents = events.filter(e => e.VALID === 1 || e.VALID === 4 || e.VALID === 5 || e.VALID === 7 || e.VALID === 8);
  const eventIds = validEvents.map(e => e.ID);
  const shuffledEventIds: number[] = random.Shuffle(eventIds);

  const eventPile: string[] = [];
  const eventInstancesLocal: Record<string, EventInstance> = {};
  for (let idx = 0; idx < shuffledEventIds.length; idx++) {
    const eId = shuffledEventIds[idx]!;
    const instanceId = `event_${eId}_${idx}`;
    eventInstancesLocal[instanceId] = { instanceId, staticId: eId };
    eventPile.push(instanceId);
  }
  ...
  return {
    ...
    cardInstances,
    monsterInstances,
    eventInstances: eventInstancesLocal,
    npcInstances: npcInstancesLocal,
    skillInstances: {},
    combat: null,
    event: {
